...
/*---------------------------------------------------------------------------*/
PROCESS(example_unicast_process, "Client 1 - Random Measurement");
...
PROCESS_THREAD(example_unicast_process, ev, data)
{
  ...
  while (i < MAX_NUM)
  {
    ...
    unsigned short randomValue = random_rand() % 22000; // Değerleri 0-21999 aralığına çekelim
    reportData.data = (float)(randomValue % 22) + ((float)(randomValue % 1000) / 1000.0);
